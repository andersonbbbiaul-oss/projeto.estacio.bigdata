...
ARQ_ACIDENTES   = os.path.join(PASTA, "acidentes_consolidado.parquet")
ARQ_LOCALIDADES = os.path.join(PASTA, "localidades_consolidado.parquet")
ARQ_VEICULOS    = os.path.join(PASTA, "veiculos_consolidado.parquet")
ARQ_VITIMAS     = os.path.join(PASTA, "vitimas_consolidado.parquet")
...
# ============================================================
# ETAPA 0 – VALIDAÇÃO PRÉVIA DAS COLUNAS
# ============================================================
print("=" * 70)
...
def validar_colunas(arquivo, nome, colunas_esperadas):
    ...
    return len(faltam) == 0 and len(extras) == 0
...
# ============================================================
print("\n" + "=" * 70)
print("VERIFICAÇÃO COMPLETA – CONSOLIDADOS 2022 (29 itens)")
print("=" * 70)
...
# ============================================================
# 2. Carregamento de chaves (PyArrow)
# ============================================================
...
# ============================================================
# 3. DataFrames específicos (pandas)
# ============================================================
df_ac = pd.read_parquet(ARQ_ACIDENTES, columns=COLUNAS_ACIDENTES)
df_ve = pd.read_parquet(ARQ_VEICULOS, columns=COLUNAS_VEICULOS)
df_vi = pd.read_parquet(ARQ_VITIMAS, columns=COLUNAS_VITIMAS)
df_lo = pd.read_parquet(ARQ_LOCALIDADES, columns=COLUNAS_LOCALIDADES)

# ============================================================
# VERIFICAÇÕES (29 itens)
# ============================================================

# --- 1. Integridade referencial (4) ---
print("\n🔗 Integridade referencial...")
check(ids_ve.issubset(ids_ac), "Veículos → Acidentes (num_acidente) OK",
      f"Existem {len(ids_ve - ids_ac)} IDs em Veículos que não estão em Acidentes")
check(ids_vi.issubset(ids_ac), "Vítimas → Acidentes (num_acidente) OK",
      f"Existem {len(ids_vi - ids_ac)} IDs em Vítimas que não estão em Acidentes")
check(chv_ac.issubset(chv_lo), "Acidentes → Localidades (chv_localidade) OK",
      f"Existem {len(chv_ac - chv_lo)} chaves em Acidentes que não estão em Localidades")
check(pares_ac.issubset(pares_lo), "Acidentes → Localidades (chv+ano) OK",
      f"Existem {len(pares_ac - pares_lo)} combinações chv+ano em Acidentes que não estão em Localidades")

# --- 2. Unicidade de chaves (4) ---
print("\n🔑 Unicidade...")
check(df_ac['num_acidente'].is_unique, "num_acidente único em Acidentes", "num_acidente duplicado em Acidentes")
dup_ve = df_ve.duplicated(subset=['num_acidente', 'tipo_veiculo']).sum()
check(dup_ve == 0, "Par (num_acidente, tipo_veiculo) único em Veículos", f"{dup_ve} duplicatas em Veículos")
dup_lo = df_lo.duplicated(subset=['chv_localidade', 'ano_referencia']).sum()
check(dup_lo == 0, "Par (chv_localidade, ano_referencia) único em Localidades", f"{dup_lo} duplicatas em Localidades")
dup_vi = df_vi.duplicated().sum()
check(dup_vi == 0, "Vítimas sem duplicatas exatas", f"Vítimas: {dup_vi} linhas duplicadas exatas (alertando, sem remover)")

# --- 3. Nulos (4 tabelas) ---
print("\n🚫 Nulos...")
for df, nome in [(df_ac, "Acidentes"), (df_lo, "Localidades"), (df_ve, "Veículos"), (df_vi, "Vítimas")]:
    nulos = df.isnull().sum()
...
check(mask_corr.all(), "qtde_envolvidos >= feridos+obitos em todos os registros",
...
check(negativos == 0, "Nenhum valor negativo em colunas numéricas", f"{negativos} linhas com valores negativos")

# --- 5. Consistência temporal (3) ---
print("\n📅 Consistência temporal...")
anos_validos = set(range(2018, 2024))
anos_presentes = set(df_ac['ano_acidente'].unique())
check(anos_presentes == anos_validos, "Anos de 2018 a 2022 presentes",
      f"Anos encontrados: {sorted(anos_presentes)} (esperado {sorted(anos_validos)})")
meses_presentes = set(df_ac['mes_acidente'].unique())
check(meses_presentes == set(range(1,13)), "Meses de 1 a 12 presentes",
...
check((mes_da_data == df_ac['mes_acidente']).all(), "Mês extraído da data coincide com mes_acidente",
      "Existem divergências entre data e mes_acidente")

# --- 6. Domínios de categorias (6) ---
print("\n🏷️ Domínios...")
check(df_ac['uf_acidente'].nunique() == 27, "27 UFs em Acidentes", f"{df_ac['uf_acidente'].nunique()} UFs em Acidentes")
check(df_lo['uf'].nunique() == 27, "27 UFs em Localidades", f"{df_lo['uf'].nunique()} UFs em Localidades")
check(df_ac['tp_acidente'].nunique() == 16, "16 tipos de acidente", f"{df_ac['tp_acidente'].nunique()} tipos")
check(df_ac['dia_semana'].nunique() == 7, "7 dias da semana", f"{df_ac['dia_semana'].nunique()} dias")
check(df_ac['fase_dia'].nunique() == 6, "6 fases do dia", f"{df_ac['fase_dia'].nunique()} fases")
check(df_ac['cond_meteorologica'].nunique() == 11, "11 condições meteorológicas",
      f"{df_ac['cond_meteorologica'].nunique()} condições")

# --- 7. Cobertura entre tabelas (3) ---
print("\n📊 Cobertura...")
sem_veic = len(ids_ac - ids_ve)
check(sem_veic == 0, f"Acidentes sem veículos: {sem_veic} (0.00%)", f"Acidentes sem veículos: {sem_veic}")
sem_vit = len(ids_ac - ids_vi)
check(sem_vit == 0, "Todos os acidentes têm vítimas", f"Acidentes sem vítimas: {sem_vit}")
check(chv_ac.issubset(chv_lo), "Todos os acidentes possuem localidade (chv_localidade presente em Localidades)",
      f"Existem {len(chv_ac - chv_lo)} chaves de acidentes sem localidade")

# --- 8. Distribuições rápidas (5) ---
print("\n📈 Distribuições...")
