...
# --- 1. Listar todos os arquivos de vítimas e seus prefixos numéricos ---
print("📂 Procurando arquivos de Vitimas em 2022...")
arquivos_mes = []   # tupla (mes_num, nome_arquivo, caminho_completo)
for arq in os.listdir(PASTA_2022):
    if not arq.endswith('.csv'):
        continue
    partes = arq.split(" - ", 1)
    if len(partes) == 2 and partes[1].startswith("Vitimas"):
        try:
            mes = int(partes[0].strip())
            caminho = os.path.join(PASTA_2022, arq)
            arquivos_mes.append((mes, arq, caminho))
        except ValueError:
            pass

if not arquivos_mes:
    print("❌ Nenhum arquivo de Vitimas encontrado.")
    exit()

# Ordena por mês (crescente) e depois inverte para processar do mais recente
