...
# --- 1. Listar todos os arquivos de acidentes e seus prefixos numéricos ---
print("📂 Procurando arquivos de Acidentes em 2024...")
arquivos_mes = []
for arq in os.listdir(PASTA_2024):
    if not arq.endswith('.csv'):
        continue
    partes = arq.split(" - ", 1)
    if len(partes) == 2 and partes[1].startswith("Acidentes"):
        try:
...
if not arquivos_mes:
    print("❌ Nenhum arquivo de Acidentes encontrado.")
    exit()
